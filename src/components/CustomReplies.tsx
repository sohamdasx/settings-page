import React, { useState } from "react";
import { CustomReply } from "../types/interfaces";
import { Search, X } from "lucide-react";

interface CustomRepliesProps {
  customReplies: CustomReply[];
  setCustomReplies: React.Dispatch<React.SetStateAction<CustomReply[]>>;
}

const CustomReplies: React.FC<CustomRepliesProps> = ({
  customReplies,
  setCustomReplies,
}) => {
  const [replyInput, setReplyInput] = useState<string>("");

  const addReply = () => {
    if (replyInput.trim()) {
      const newReply = { id: Date.now().toString(), text: replyInput };
      setCustomReplies([...customReplies, newReply]);
      setReplyInput("");
    }
  };

  const removeReply = (id: string) => {
    setCustomReplies(customReplies.filter((reply) => reply.id !== id));
  };

  return (
    <div className="my-8">
      <h2 className="text-xl font-semibold mb-3">Custom Replies</h2>
      <p className="text-gray-600 mb-3">
        These are custom replies which you can send to users instantly.
      </p>

      <div className="mt-4">
        <div className="flex items-center">
          <div className="relative flex-grow">
            <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <Search size={16} className="text-gray-500" />
            </div>
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              placeholder="Add replies"
              value={replyInput}
              onChange={(e) => setReplyInput(e.target.value)}
              onKeyDown={(e) =>
                e.key === "Enter" && replyInput.trim() && addReply()
              }
            />
          </div>
          {replyInput.trim() && (
            <button
              type="button"
              className="ml-3 px-4 py-2 bg-black text-white rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
              onClick={addReply}
            >
              Add
            </button>
          )}
        </div>

        <div className="flex flex-wrap gap-2 mt-3">
          {customReplies.map((reply) => (
            <div
              key={reply.id}
              className="flex items-center bg-gray-100 px-3 py-1 rounded-full"
            >
              <span className="text-sm">{reply.text}</span>
              <button
                type="button"
                className="ml-2 text-gray-500 hover:text-gray-700 focus:outline-none"
                onClick={() => removeReply(reply.id)}
              >
                <X size={16} className="cursor-pointer" />
              </button>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default CustomReplies;